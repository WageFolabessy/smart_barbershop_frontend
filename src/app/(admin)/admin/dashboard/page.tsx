...
export default function AdminDashboard() {
    ...
    const arrayFrom = (src: unknown, keys: string[] = []): unknown[] => {
        ...
        const values =
            (src['values'] as unknown) ||
            (src['totals'] as unknown) ||
            (src['amounts'] as unknown)
        if (
            ...
            return labels.map((l, i) => ({
                name: String(l),
                total: typeof values[i] === 'number' ? values[i] : 0,
            }))
        }
        return []
    }
    ...
    const normalizeRevenue = (raw: unknown): RevenueChartItem[] => {
        ...
        const revenueObj =
            env &&
            (isRecord(env['revenue'])
                ? (env['revenue'] as Record<string, unknown>)
                : isRecord(env)
                  ? env
                  : undefined)
        if (revenueObj) {
            return [
                ...
                {
                    name: 'Minggu ini',
                    total: safeNumber(revenueObj['this_week'], 0),
                },
                {
                    name: 'Bulan ini',
                    total: safeNumber(revenueObj['this_month'], 0),
                },
                {
                    name: 'Tahun ini',
                    total: safeNumber(revenueObj['this_year'], 0),
                },
            ]
        }
        ...
        return revenueArr.map((item, idx) => {
            if (isRecord(item)) {
                ...
                const total = safeNumber(
                    item['total'] ?? item['amount'] ?? item['value'] ?? 0,
                    0
                )
                return { name, total }
            }
    ...
    const normalizeBarberPerformance = (
        raw: unknown
        ...
        return arr.map((item) => {
            if (isRecord(item)) {
                const name =
                    typeof item['barber_name'] === 'string'
                        ? (item['barber_name'] as string)
                        : typeof item['name'] === 'string'
                          ? (item['name'] as string)
                          : typeof item['label'] === 'string'
                            ? (item['label'] as string)
                            : '-'
                const total_bookings = safeNumber(
                    item['total_bookings'] ??
                        item['bookings_count'] ??
                        item['count'] ??
                        0,
                ...
                const completed_bookings = safeNumber(
                    item['completed_bookings'] ?? item['completed'] ?? 0,
                    0
                )
    ...
    const normalizeRecentBookings = (raw: unknown): RecentBookingItem[] => {
        ...
        return arr.map((item) => {
            if (isRecord(item)) {
                const id =
                    typeof item['id'] === 'number' ? (item['id'] as number) : 0
                const customer_name =
                ...
                const booking_datetime =
                    typeof item['booking_datetime'] === 'string'
                        ? (item['booking_datetime'] as string)
    ...
    const popularServices: PopularServiceItem[] =
        normalizePopularServices(popularServicesRaw)
    const topService: PopularServiceItem | null =
        popularServices.length > 0
            ? popularServices.reduce(
                  (max, cur) => (cur.count > max.count ? cur : max),
                  popularServices[0]
              )
            : null

    const barberPerformance: BarberPerformanceItem[] =
        normalizeBarberPerformance(barberPerformanceRaw)

    const recentBookingsArr: RecentBookingItem[] =
        normalizeRecentBookings(recentBookings)

    let totalRevenue = safePick(
        overview,
        ['total_revenue', 'revenue_total', 'sum_revenue'],
        0
    )
    const revenueEnvelope =
        isRecord(revenueRaw) && isRecord(revenueRaw['data'])
            ? (revenueRaw['data'] as Record<string, unknown>)
            : undefined
    if (
        !totalRevenue &&
        revenueEnvelope &&
        isRecord(revenueEnvelope['revenue'])
    ) {
        totalRevenue = safeNumber(
            (revenueEnvelope['revenue'] as Record<string, unknown>)[
                'this_year'
            ],
            0
        )
    ...
    const totalBookings = isRecord(overview)
        ? safeNumber(
              overview['total_bookings'] && isRecord(overview['total_bookings'])
                  ? (overview['total_bookings'] as Record<string, unknown>)[
                        'all'
                    ]
        ...
        : 0
    const activeUsers = isRecord(overview)
        ? safeNumber(
              overview['total_users'] && isRecord(overview['total_users'])
                  ? (overview['total_users'] as Record<string, unknown>)['all']
    ...
    const bookingsStatusObject =
        isRecord(overview) && isRecord(overview['total_bookings'])
            ? (overview['total_bookings'] as Record<string, unknown>)
            : undefined
        ...
        ? [
              {
                  name: 'Menunggu',
                  key: 'pending',
                  value: safeNumber(bookingsStatusObject['pending'], 0),
              },
              {
                  name: 'Terkonfirmasi',
                  key: 'confirmed',
                  value: safeNumber(bookingsStatusObject['confirmed'], 0),
              },
              {
                  name: 'Selesai',
                  key: 'completed',
                  value: safeNumber(bookingsStatusObject['completed'], 0),
              },
              {
                  name: 'Dibatalkan',
                  key: 'cancelled',
                  value: safeNumber(bookingsStatusObject['cancelled'], 0),
              },
          ]
        : []
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 max-[380px]:grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
                <Card>
                ...
                <Card className="col-span-3">
                    ...
                    <CardContent>
                        <div className="space-y-8">
                            {recentBookingsArr.map(
                                (booking: RecentBookingItem) => (
                                    <div
                                        key={booking.id}
                ...
                <Card>
                    ...
                    <CardContent>
                        ...
                        >
                            <BarChart
                                data={
                                    Array.isArray(barberPerformance)
                                        ? barberPerformance
                                        : []
                                }
                                layout="vertical"
